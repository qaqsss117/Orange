...
mod tests {
    ...
    use super::{
        RuleResourceError, RuleResourceId, RuleResourceStore, SharedRuleResourceRootVerifier,
        unsafe_path_flags,
    };

    const SRS: &[u8] = b"SRS\x02fixture";

    struct AcceptSharedRoot;

    impl SharedRuleResourceRootVerifier for AcceptSharedRoot {
        fn verify_shared_rule_root(
            &self,
            _canonical_root: &std::path::Path,
        ) -> Result<(), RuleResourceError> {
            Ok(())
        }
    }

    struct RejectSharedRoot;

    impl SharedRuleResourceRootVerifier for RejectSharedRoot {
        fn verify_shared_rule_root(
            &self,
            _canonical_root: &std::path::Path,
        ) -> Result<(), RuleResourceError> {
    ...
    fn fixture() -> (TempDir, Vec<u8>) {
        ...
        fs::write(directory.path().join("geoip-cn.srs"), SRS).unwrap();
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
            fs::set_permissions(
                directory.path().join("geoip-cn.srs"),
                fs::Permissions::from_mode(0o644),
            )
            .unwrap();
            fs::set_permissions(directory.path(), fs::Permissions::from_mode(0o755)).unwrap();
        }
        let hash = format!("{:x}", Sha256::digest(SRS));
        let manifest = json!({
            "schema_version": 1,
            "manifest_id": "compat-v1",
            "resources": [{
                "id": "geoip-cn",
                "name": "geoip-cn.srs",
                "format": "srs",
        ...
        (directory, serde_json::to_vec(&manifest).unwrap())
    }

    fn mutate(document: &[u8], mutation: impl FnOnce(&mut Value)) -> Vec<u8> {
    ...
    #[test]
    fn logical_id_resolves_only_after_full_manifest_validation() {
        let (directory, document) = fixture();
        let store = RuleResourceStore::open_user_private(directory.path()).unwrap();
        let activation = store.activate_manifest(&document).unwrap();
        assert_eq!(activation.manifest_id(), "compat-v1");
        assert_eq!(activation.resource_count(), 1);
        let resolved = store
    ...
    #[test]
    fn path_like_ids_and_case_ambiguity_are_rejected() {
        for value in [
            "../geoip-cn",
            "file://geoip-cn",
            "C:\\geoip-cn",
            ...
            assert_eq!(
                RuleResourceId::new(value),
                Err(RuleResourceError::InvalidResourceId)
            );
        }
        let (directory, document) = fixture();
        let store = RuleResourceStore::open_user_private(directory.path()).unwrap();
        let bad_name = mutate(&document, |value| {
            value["resources"][0]["name"] = json!("GeoIP-CN.srs");
        });
        assert_eq!(
            store.activate_manifest(&bad_name),
            Err(RuleResourceError::InvalidManifest)
        );
    }

    #[test]
    fn size_hash_and_format_failures_preserve_the_active_manifest() {
        let (directory, document) = fixture();
        let store = RuleResourceStore::open_user_private(directory.path()).unwrap();
        store.activate_manifest(&document).unwrap();

        let bad_size = mutate(&document, |value| {
            value["manifest_id"] = json!("bad-size");
            value["resources"][0]["size_bytes"] = json!(SRS.len() + 1);
        });
        assert_eq!(
            store.activate_manifest(&bad_size),
            Err(RuleResourceError::SizeMismatch)
        );
        let bad_hash = mutate(&document, |value| {
            value["manifest_id"] = json!("bad-hash");
            value["resources"][0]["sha256"] = json!("0".repeat(64));
        });
        assert_eq!(
            store.activate_manifest(&bad_hash),
            Err(RuleResourceError::HashMismatch)
        );
        let bad_format = mutate(&document, |value| {
            value["manifest_id"] = json!("bad-format");
            value["resources"][0]["format_version"] = json!(1);
        });
        assert_eq!(
            store.activate_manifest(&bad_format),
            Err(RuleResourceError::FormatMismatch)
        );
        assert_eq!(
            store.active_manifest_id().unwrap().as_deref(),
            Some("compat-v1")
        );
        assert!(
            store
    ...
    #[test]
    fn post_activation_resource_changes_fail_closed_without_replacing_state() {
        let (directory, document) = fixture();
        let store = RuleResourceStore::open_user_private(directory.path()).unwrap();
        store.activate_manifest(&document).unwrap();

        fs::write(directory.path().join("geoip-cn.srs"), b"SRS\x02changed").unwrap();
        assert_eq!(
            store.resolve(&RuleResourceId::new("geoip-cn").unwrap()),
            Err(RuleResourceError::HashMismatch)
        );
        assert_eq!(
            store.active_manifest_id().unwrap().as_deref(),
    ...
    #[test]
    fn unknown_fields_duplicate_resources_and_unsafe_names_are_rejected() {
        let (directory, document) = fixture();
        let store = RuleResourceStore::open_user_private(directory.path()).unwrap();
        let unknown = mutate(&document, |value| value["open"] = json!(true));
        assert_eq!(
            store.activate_manifest(&unknown),
            Err(RuleResourceError::InvalidManifest)
        );
        let duplicate = mutate(&document, |value| {
            let duplicate = value["resources"][0].clone();
            value["resources"].as_array_mut().unwrap().push(duplicate);
        });
        assert_eq!(
            store.activate_manifest(&duplicate),
            Err(RuleResourceError::DuplicateResource)
        );
        let traversal = mutate(&document, |value| {
            value["resources"][0]["name"] = json!("../geoip-cn.srs");
        });
        assert_eq!(
            store.activate_manifest(&traversal),
            Err(RuleResourceError::InvalidManifest)
        );
        for name in ["geoip cn.srs", "geoip_cn.srs", "geoip-cn\u{7f}.srs"] {
            let invalid_name = mutate(&document, |value| {
                value["resources"][0]["name"] = json!(name);
            });
            assert_eq!(
                store.activate_manifest(&invalid_name),
                Err(RuleResourceError::InvalidManifest)
            );
        }
        let invalid_time = mutate(&document, |value| {
            value["resources"][0]["generated_at"] = json!("2026-02-31T00:00:00Z");
        });
        assert_eq!(
            store.activate_manifest(&invalid_time),
            Err(RuleResourceError::InvalidManifest)
        );
    }

    #[test]
    fn shared_store_requires_platform_permission_verification() {
        let (directory, document) = fixture();
        assert!(matches!(
            RuleResourceStore::open_service_shared(directory.path(), Arc::new(RejectSharedRoot)),
            Err(RuleResourceError::PermissionDenied)
        ));
        let store =
            RuleResourceStore::open_service_shared(directory.path(), Arc::new(AcceptSharedRoot))
                .unwrap();
        store.activate_manifest(&document).unwrap();
    }

    #[test]
    fn symlink_and_windows_reparse_flags_share_the_same_fail_closed_path() {
        assert!(unsafe_path_flags(true, 0));
        assert!(unsafe_path_flags(false, 0x0400));
        assert!(!unsafe_path_flags(false, 0));
    }

    #[cfg(unix)]
    #[test]
    fn executable_resource_and_symlinked_resource_are_rejected() {
        use std::os::unix::fs::{PermissionsExt, symlink};

        let (directory, document) = fixture();
        let resource = directory.path().join("geoip-cn.srs");
        fs::set_permissions(&resource, fs::Permissions::from_mode(0o755)).unwrap();
        let store = RuleResourceStore::open_user_private(directory.path()).unwrap();
        assert_eq!(
            store.activate_manifest(&document),
            Err(RuleResourceError::PermissionDenied)
        );

        fs::remove_file(&resource).unwrap();
        let target = directory.path().join("target.srs");
        fs::write(&target, SRS).unwrap();
        symlink(&target, &resource).unwrap();
        assert_eq!(
            store.activate_manifest(&document),
            Err(RuleResourceError::UnsafePath)
        );
    }
}
